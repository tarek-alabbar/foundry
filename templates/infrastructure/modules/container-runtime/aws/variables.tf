...
variable "resource_group_name" {
  type            = string
  description     = "Maps to AWS: not used, kept for interface parity"
}
variable "location" {
  type            = string
...
variable "cpu" {
  type            = string
  default         = "512"
  description     = "ECS CPU units (256/512/1024/2048/4096)"
}
variable "memory" {
  type            = string
  default         = "1024"
  description     = "ECS memory in MiB (512/1024/2048/4096)"
}
variable "env_vars" {
  type            = map(string)
  default         = {}
...
variable "environment_id" {
  type            = string
  description     = "Maps to AWS: VPC ID"
}
